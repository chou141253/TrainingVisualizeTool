...
class Canvas(object):
    ...
    def data2plotpos(self, x, y, xtype='batch', ytype='loss'):
        ...
        elif ytype in ['e', 'epoch']:
            x = int((x*self.epoch_pixel) + self.blank_size/2)
    ...
    def draw_bg_x(self, bg, line_type):
        ...
        if line_type == "first":
            ...
            while x_pos<((self.total_w-self.blank_size/2)+self.epoch_pixel/draw_line_in_epoch):
        ...
        if line_type == "second":
            ...
            epoch_count = 0
            while x_pos<((self.total_w-self.blank_size/2)+self.epoch_pixel):
    ...
    def draw_bg_y(self, bg, line_type):
        ...
        if line_type == "first":
            ...
            while y_pos>(self.blank_size/2 - self.h/(self.max_vis_loss*5*draw_line_in_loss)):
        ...
        if line_type == "second":
            ...
            point_count = 0
            while y_pos>(self.blank_size/2 - self.h/(self.max_vis_loss*5)):
    ...
    def plot_list(self, dlist, showval_list, dot_color=(200,0,0)):
        """
        input dlist_x and dlist_y to plot.
        """
        prvs_p, this_p = None, None
        for i in range(len(dlist)):
            this_p = self.data2plotpos(x=i, y=dlist[i], xtype='batch', ytype='loss')
